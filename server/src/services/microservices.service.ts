...
export class MicroservicesService {
  ...
  async onBootstrap(app: ArgOf<'app.bootstrap'>) {
    ...
    await this.jobService.init({
      ...
      [JobName.LIBRARY_SCAN_REMOVED]: (data) => this.libraryService.handleQueueScanRemoved(data),
      [JobName.LIBRARY_DELETE]: (data) => this.libraryService.handleDeleteLibrary(data),
      [JobName.LIBRARY_CHECK_OFFLINE]: (data) => this.libraryService.handleCheckAssetOnlineStatus(data),
