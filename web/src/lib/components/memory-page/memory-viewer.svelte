<script lang="ts">
  ...
  $: memories = buildMemoryAssets($memoryStore);
  $: current = init(memories, $page);
  $: handlePromiseError(handleProgress($progress));
  $: handlePromiseError(handleAction(galleryInView ? 'pause' : 'play'));

  const buildMemoryAssets = (memories?: MemoryLaneResponseDto[]) => {
    memories = memories ?? [];
  ...
  const findById = (assetId?: string) => memories.find((item) => item.asset.id === assetId);
  const init = (memoryAssets: MemoryAsset[], page: typeof $page) => {
    const assetId = page.params?.assetId ?? page.url.searchParams.get(QueryParameter.ID) ?? undefined;
    return findById(assetId) ?? memoryAssets[0];
  };
  const asHref = (asset: AssetResponseDto) => `?${QueryParameter.ID}=${asset.id}`;
  const handleNavigate = async (asset?: AssetResponseDto, reset?: boolean) => {
    if (reset) {
  ...
  const handlePreviousAsset = () => handleNavigate(current?.previous?.asset, true);
  ...
  const handleProgress = async (progress: number) => {
    ...
    if (progress === 1) {
      await handleAction(current?.next ? 'reset' : 'pause');
  ...
  const onAssetDelete = (assetIds: string[]) => {
    ...
    current.memory.assets = current.memory.assets.filter((a: AssetResponseDto) => !assetIdSet.has(a.id));
    memories = buildMemoryAssets($memoryStore);
  };
</script>
...
{#if isMultiSelectionMode}
  <div class="sticky top-0 z-[90]">
    <AssetSelectControlBar assets={selectedAssets} clearSelect={() => (selectedAssets = new Set())}>
      ...
      <FavoriteAction removeFavorite={isAllFavorite} onFavorite={onAssetUpdate} />

      <ButtonContextMenu icon={mdiDotsVertical} title={$t('add')}>
        <DownloadAction menuItem />
        <ChangeDate menuItem />
        <ChangeLocation menuItem />
        <ArchiveAction menuItem unarchive={isAllArchived} onArchive={onAssetUpdate} />
            ...
            <a class="relative w-full py-2" href={asHref(asset)}>
              ...
              {#await resetPromise}
                <span
                  ...
                  style:width={`${index < current.assetIndex ? 100 : 0}%`}
                />
              {:then}
                <span
                  class="absolute left-0 h-[2px] bg-white"
